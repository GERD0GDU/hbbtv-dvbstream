...
int enqueue_fd(FILE* pf, char* filename, int bps, char close_fd) {
...
int main(int argc, char *argv[])
{
	...
	while (i < (unsigned)argc) {
		if (argv[i][0] == 'b' && argv[i][1] == ':') {
			bps = atoi(&(argv[i][2]));
			pf = fopen(argv[i + 1], "rb");
			if (bps > 0 && pf != NULL) {
				if (enqueue_fd(pf, argv[i + 1], bps, 0))
					return 2;
			}
			else if (bps < 0){
			...
			pf = fopen(argv[i + 1], "rb");
			if (bps > 0 && pf != NULL) {
				if (enqueue_fd(pf, argv[i + 1], bps, 1))
					return 2;
			}
			else if (bps < 0){
				fprintf(stderr, "specified bps is not positive\n");
			...
			pf = fopen(argv[i + 1], "rb");
			if (bps > 0 && pf != NULL) {
				if (enqueue_fd(pf, argv[i + 1], bps - (int)output_bitrate, 0))
					return 2;
				output_bitrate = bps;
			}
			else if (bps < 0){
	...
	while (1) {
		...
		/* read packet */
		byte_read = 0;
		byte_read = fread(ts_packet, 1, TS_PACKET_SIZE, chosen_fd->pf);
		if (byte_read < TS_PACKET_SIZE) {
			fclose(chosen_fd->pf);
			if (chosen_fd->close_fd) {
				return 0;
			}
			chosen_fd->pf = fopen(chosen_fd->filename, "rb");
			if (chosen_fd->pf == NULL) {
				fprintf(stderr, "Can't find file %s\n", chosen_fd->filename);
				return 2;
			}
			byte_read = fread(ts_packet, 1, TS_PACKET_SIZE, chosen_fd->pf);
