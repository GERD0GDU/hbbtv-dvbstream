...
int main(int argc, char *argv[])
{
	...
	if (argc >= 7) {
		...
		current_file_pes = fopen(argv[6], "rb");
		file_pes[0] = current_file_pes;
		_snprintf(filelength, PATH_MAX, "%s.length", argv[6]);
	...
	for (i = 7; i < argc; i++) {
		struct stat file_stat;
		if (stat(argv[i], &file_stat) == 0) { /* take stats of the real file not of the symbolic link */
			if (S_ISFIFO(file_stat.st_mode)) {
	...
	int packets_to_start = (int)((pts_offset * bitrate) / (PTS_TIME * 8 * TS_PACKET_SIZE));/* packets to pts */
				...
				if (byte_read <= 0 && loop_on) {
					...
					if (argc >= 6 + open_counter) {
						...
						if (current_file_pes == NULL) {
							...
							_snprintf(filelength, PATH_MAX, "%s.length", argv[6 + open_counter]);
						}
					}
					...
					*/
					...
					int null_packets = (int)((left_packet + (next_pts_rounded_to_next_video_frame * bitrate)) / (PTS_TIME * 8 * TS_PACKET_SIZE)); /* packets to next pts */
					null_packets -= (int)ts_packets; /* packets sent */
