...
u_char* get_section(u_short* section_size, FILE* fd)
{
	u_short	temp = 0;
	u_char* section = 0;
	...
	if (section_size) {
		...
		/* Read first bytes */
		*section_size = 0;
		if (fread(g_section_head, 1, SECTION_HEADER_SIZE, fd)) {
			...
			*section_size = temp;
			...
			if (*section_size <= (SECTION_MAX_SIZE - SECTION_HEADER_SIZE)) {
				...
				fread(g_section + SECTION_HEADER_SIZE, 1, *section_size, fd);
				section = g_section;
